...
//         m_weight{ static_cast<double>(weight) }
//     { }
...
//         increment.apply(source, mapped);

//         residuals[0] = m_weight * (mapped[0] - target[0]);
// 		residuals[1] = m_weight * (mapped[1] - target[1]);
// 		residuals[2] = m_weight * (mapped[2] - target[2]);
...
//         T mapped[3];
//         T source[3] = { T(m_sourcePoint(0)),
//                         T(m_sourcePoint(1)),
//                         T(m_sourcePoint(2))};

//         increment.apply(source, mapped);

//         Eigen::Matrix<T, 3, 1> targetNormal = m_targetNormal.cast<T>();
//         Eigen::Matrix<T, 3, 1> targetPoint = m_targetPoint.cast<T>();
//         Eigen::Matrix<T, 3, 1> mappedVec(mapped[0], mapped[1], mapped[2]);

//         residuals[0] = targetNormal.transpose().dot(mappedVec - targetPoint);
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                float angle = acos(sourceNormal.transpose() * targetNormal);
                match.weight = angle > 60 ? -1 : match.weight;
                match.idx = angle > 60 ? -1 : match.idx;
            }
        }
    }
};
...
//                 problem.AddResidualBlock(PointToPointConstraint::create(sourcePoint, targetPoint, 1.0f),
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            RowVectorXf planeRow;
            planeRow << n(2)* s(1) - n(1) * s(2) , n(0) * s(2) - n(2) * s(0), n(1) * s(0) - n(0) * s(1),
                    n(0), n(1), n(2);
            A.block<1, 6>(i* 4, 0) = planeRow;
            b(i * 4) = n(0) * d(0) + n(1) * d(1) + n(2) * d(2) - n(0) * s(0) - n(1) * s(1) - n(2) * s(2);

            // TODO: Add the point-to-point constraints to the system
            MatrixXf pointRows(3,6);
            pointRows << 0, s(1), -s(2), 1, 0, 0,
                    -s(2), 0, s(0), 0, 1, 0,
                    s(1), -s(0), 0, 0, 0, 1;
            A.block<3,6>(i*4+1, 0) = pointRows;

            b.block<3,1>(i*4+1, 0) = d - s;
        ...
        VectorXf x(6);
        FullPivLU<Matrix<float, Dynamic, Dynamic>>  lu(A.transpose() * A);
        x = lu.solve(b);
